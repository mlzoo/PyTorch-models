...
parser.add_argument('-n', '--model',  default='plainet', help='The model type')
parser.add_argument('-d', '--depth', default='18', help='The model depth 18/34' )
...
transform = transforms.Compose([
        ...
        transforms.RandomResizedCrop(32), # 随机剪裁
...
if model_name == 'plainet':
    model = PlainNetCifar10(num_classes=10, depth=depth).to(device)
elif model_name == 'resnet':
    model = ResNetCifar10(num_classes=10, depth=depth).to(device)
...
#            },
#            './plainnet-cifar10--epoch-' + str('%03d') % (epoch + 1) + '--accuracy-' + str("%.4f" % epoch_val_accuracy.item()) + '.pth.tar')
#     print('Model saved at', './plainnet-cifar10--epoch-' + str('%03d') % (epoch + 1) + '--accuracy-' + str("%.4f" % epoch_val_accuracy.item()) + '.pth.tar')
